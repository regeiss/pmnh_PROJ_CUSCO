import 'dart:convert';
import '../../usuario/data/usuario_data.dart';
...
final setAuthenticatedUserProvider = StateProvider.family<void, UsuarioModel>(
  (ref, userdata) async {
    final prefs = await ref.watch(sharedPrefProvider);
    ref.watch(setAuthStateProvider);
    prefs.setString(
...
final getAuthenticatedUserProvider = FutureProvider<UsuarioModel>(
  (ref) async {
    final prefs = await ref.watch(sharedPrefProvider);
    ref.watch(setAuthStateProvider);
    dynamic user =
