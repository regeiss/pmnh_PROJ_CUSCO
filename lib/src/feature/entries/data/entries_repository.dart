...
class EntriesRepository {
  ...
  // delete
  Future<void> deleteEntry({required UserID uid, required EntryID entryId}) => _firestore.doc(entryPath(uid, entryId)).delete();

  // read
  Stream<List<Entry>> watchEntries({required UserID uid, JobID? jobId}) =>
      queryEntries(uid: uid, jobId: jobId).snapshots().map((snapshot) => snapshot.docs.map((doc) => doc.data()).toList());
...
final jobEntriesQueryProvider = Provider.autoDispose.family<Query<Entry>, JobID>((ref, jobId) {
