...
class JobsRepository {
  ...
  // create
  Future<void> addJob({required UserID uid, required String name, required int ratePerHour}) => _firestore.collection(jobsPath(uid)).add({
        'name': name,
        'ratePerHour': ratePerHour,
      });

  // update
  Future<void> updateJob({required UserID uid, required Job job}) => _firestore.doc(jobPath(uid, job.id)).update(job.toMap());
  ...
  Stream<List<Job>> watchJobs({required UserID uid}) =>
      queryJobs(uid: uid).snapshots().map((snapshot) => snapshot.docs.map((doc) => doc.data()).toList());
